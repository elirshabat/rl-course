...
def value_iteration(mdp, gamma, nIt):
    ...
    for it in range(nIt):
        ...
        pi = np.zeros(mdp.nS, dtype='int')  # REPLACE THIS LINE WITH YOUR CODE

        for s in np.arange(mdp.nS):
            A = [sum([t[0]*(t[2] + gamma*Vprev[t[1]]) for t in mdp.P[s][a]]) for a in np.arange(mdp.nA)]
            pi[s] = int(np.argmax(A))
            V[s] = A[pi[s]]
        # End of our code
