...
class Net(nn.Module):
    ...
    def forward(self, x):
        u1 = self.fc1(x)
        v1 = torch.nn.functional.relu(u1)
        out = self.fc2(v1)
        return out
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        print("epoch:{} btach:{} --> loss:{}".format(epoch, i, loss.data.numpy()))
...
torch.save(net.state_dict(), os.path.join(model_dir, '{}_model.pkl'.format('deep')))

with open("out/mnist_deep_output_data.json", 'wt') as f:
