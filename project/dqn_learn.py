...
def dqn_learing(
    ...
    for t in count():
        ...
        if (t > learning_starts and
            ...
            next_q = target_Q(Variable(next_obs).detach())
            max_next = torch.max(next_q, 1)[0]
            mask = Variable(torch.from_numpy(1.0 - done_batch).type(dtype)).detach()
            max_next = gamma*(max_next*mask)
            target_value = Variable(torch.from_numpy(reward_batch).type(dtype)).detach() + max_next
            q_value = Q(Variable(torch.from_numpy(obs_batchs).type(dtype) / 255.0).detach())
            action_batch = Variable(torch.from_numpy(action_batch).type(dtype).long().view(-1,1)).detach()
